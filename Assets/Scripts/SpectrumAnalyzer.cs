...
public class SpectrumAnalyzer : MonoBehaviour
{
    ...
    {
        isBuilding = true;
        pillarMaterial = new Material(Shader.Find("Standard"));
        CreatePillarsByShapes();
        StartCoroutine(ColorPillars());
    }

    private IEnumerator ColorPillars()
    {
        while (true)
        {
            if (spectrum == null || spectrum.Length < pillars.Count * 3)
                yield return null;

            for (int i = 0; i < pillars.Count; i++)
            {
                Renderer renderer = pillars[i].GetComponent<Renderer>();
                Material material = renderer.material;

                Color color = new Color(spectrum[i * 3], spectrum[i * 3 + 1], spectrum[i * 3 + 2]);
